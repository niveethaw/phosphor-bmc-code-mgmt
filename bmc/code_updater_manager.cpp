...
auto CodeUpdateManager::startUpdate(int fd) -> sdbusplus::async::task<bool>
{
...
auto CodeUpdateManager::run() -> sdbusplus::async::task<void>
{
    auto res = copyImage();
    if (!res)
    {
        lg2::error("Failed to copy image via CLI");
        co_return;
    }
...
bool CodeUpdateManager::run()
{
    return copyImage();
}

void CodeUpdateManager::setApplyTime()
{
...
void CodeUpdateManager::setRequestedActivation(const std::string& path)
{
    utils::PropertyValue value =
...
void CodeUpdateManager::updateActivation(sdbusplus::message_t& msg)
{
    ...
    for (auto& interface : interfaces)
    {
        ...
        try
        {
            ...
            if (imageProp == readyPro && isCLICodeUpdate)
            {
                setApplyTime();
